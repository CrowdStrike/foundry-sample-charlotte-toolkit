...
interface HomeProps {
  falcon: unknown; // Complex Falcon API types - using unknown for type safety
}

// Type guard for falcon data access
interface FalconWithData {
  data?: unknown;
}
...
const Home = React.memo(({ falcon }: HomeProps) => {
  ...
  const { availableContextOptions } = useContextProcessor({
    falconData: (falcon as FalconWithData)?.data,
  });

  // Calculate context counts for the JSON manager
  const contextCounts = {
    total: availableContextOptions.length,
    domains: availableContextOptions.filter((opt) => opt.type === 'domain')
      .length,
    files: availableContextOptions.filter((opt) => opt.type === 'file').length,
    ips: availableContextOptions.filter((opt) => opt.type === 'ip').length,
    fqdns: availableContextOptions.filter(
      (opt) => opt.type === 'domain' && opt.subType === 'fqdn',
    ).length,
  };
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const result: WorkflowExecutionResult = await executeWorkflowWithCache(
        // biome-ignore lint/suspicious/noExplicitAny: Falcon API has complex internal types that require any for workflow executor compatibility
        falcon as any,
        workflowParameters,
      );
      ...
      <SlCard className="full-width-card">
        ...
        >
          ...
          <SlTab
            ...
            className={
              !state.hasSubmittedQuery ? 'opacity-50 cursor-not-allowed' : ''
            }
          >
            {getResponseTabIndicator()}
            Response
          ...
          <SlTabPanel name="request">
            <QueryForm
              ...
              setShowJsonTab={(showJsonTab: boolean) =>
                updateState({ showJsonTab })
              }
                    ...
                    <div className="socket-info-container">
                      <div className="socket-info-grid">
                        <div className="socket-info-row">
                          <span className="socket-info-label">
                            Current Socket:
                          </span>
                          <span
                            className={
                  ...
                  <div>
                    <div className="json-section-header">
                      ...
                      >
                        ...
                        {rawResponseCopyState === 'check-circle'
                          ? 'Copied!'
                          : 'Copy Raw Response'}
                      </SlButton>
                    </div>
